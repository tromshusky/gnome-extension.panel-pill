...
const stretch_after_maximize = 400;
    ...
    enable() {
        ...
        const onWindowMaximized = (wm, win) => {
            if (win.metaWindow.get_maximized() == 3) {
                const unmax = () => win.metaWindow.unmaximize(3);
                const stretch = () => win.metaWindow.move_resize_frame(false, 0, 0, global.screen_width, global.screen_height);
                unmax();
                setTimeout(stretch, stretch_after_maximize);
            };
        };
        ...
        const make_round = () => Main.panel.set_style("border-radius: " + new_radius + "px;");
        setTimeout(make_round, 1000);
